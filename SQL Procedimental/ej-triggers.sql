...
--(no es necesario tener en cuenta las inserciones). Entiendo que el orden 1º insercion departamento sin director, 2º inserción
--empleado, 3º update de deparatmentos asignando director, debería gestionarse con procedimientos.
		...
		ELSE
			...
			IF rec.horastotales>limite THEN 
				horas_de_mas:= rec.horastotales-limite;
					...
					RAISE WARNING ' Está trabajando % horas de más.', horas_de_mas;
			ELSE
				RAISE NOTICE 'El empleado % tiene % horas asignadas.',rec.empleado, rec.horastotales;
			END IF;	
		END IF;
...
--Misma función devolviendo todos los empelados que tienen asignadas horas de más, en caso de haberlos:
...
BEGIN
	...
	LOOP	
		...
		END IF;	
	--En caso de no superar el límite de horas, devuelve una tabla vacía.
	END LOOP;
END;
$$
LANGUAGE plpgsql;
...
--Trigger para evitar que se le asignen más de 40h a un empleado:
...
BEGIN
	...
	IF TG_OP='UPDATE' THEN
		...
		IF horas_totales>limite THEN 
			--Aunque las horas resultantes nuevas sean mayores de 40, si lo que se pretende es bajar el número de horas asignadas, 
			--entonces lo permite.
			...
			--En cualquier otro caso, no se acepta el cambio.
				...
				RAISE WARNING E'El empleado % no puede trabajar más de % horas.\nNo se han asigando las horas nuevas.',
				rec.empleado, limite;
					RETURN NULL;
				END IF;
		ELSE
...
--Función para saber si a un empleado ya le han asignado un determinado proyecto:
...
BEGIN 
	...
	ELSEIF proyectoyaasignadoaempleado(id_proyecto, id_empleado) = 1 THEN --Usando la función creada más arriba, se sabe si hay que hacer un update o una inserción.
	...
	ELSE 
		--En fecha de incorporación se añaden valores por defecto para la fecha de incrporación(actual) y la de baja(null).
		INSERT INTO proyectos_empleados VALUES(id_proyecto, id_empleado, horas_asignadas,fecha_incorporacion, NULL);
...
-- Insertar un familiar: debería hacerse mediante un procedimiento que actualizase, 
--además de la tabla de familiares, la NM de relación con empleados. Las dos en una única 
--transacción.
