...
/* 8. Función que asina a un empleado horas de un proyecto. Si el empleado tiene una asignación previa
 * con el proyecto, lo actualizará con la nueva asignación de horas */
 
 
 CREATE OR REPLACE PROCEDURE asignacionproyectosempleados(
 	id_proyecto int, id_empleado int, horas_asignadas interval) AS $$
DECLARE 
BEGIN 
	INSERT INTO proyectos_empleados VALUES(id_proyecto, id_empleado, horas_asignadas, CURRENT_DATE, NULL);
	-- Si ese proyecto ya está asignado a ese empleado, el insert fallará con "SQL Error [23505]: ERROR: duplicate key..."
	EXCEPTION WHEN SQLSTATE '23505' THEN -- Capturamos ese error y actualizamos en lugar de insertar
		UPDATE proyectos_empleados SET horas=horas_asignadas, fbaja= NULL -- ***
				WHERE empleado=id_empleado AND proyecto=id_proyecto;
		RAISE WARNING 'Se han actualizado las horas que el empleado % tiene asignadas al proyecto %.', id_empleado, id_proyecto;
END; $$ LANGUAGE plpgsql;
/* *** Esto sería discutible, pero si un empleado se reincorpora a un proyecto en que ya había terminado, es razonable
"abrirlo" poniendo la fecha de baja del proyecto a NULL */

call asignacionproyectosempleados(1,200,'10h'); -- Empleado no existente, ya salta la FK
call asignacionproyectosempleados(100,10,'10h'); -- Proyecto no existente, ya salta la FK
call asignacionproyectosempleados(12,34,'10h'); -- Nueva asignación
call asignacionproyectosempleados(12,34,'16h'); -- Asignación ya existente
