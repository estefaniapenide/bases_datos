...
/* Si considerásemnos que un empleado puede tener departamento a NULL (no es el caso), habría
...
/* El LEFT JOIN no puede hacerse con WHERE directamente. Su equivalente sería: */
...
/* "UNION ALL" Junta filas aunque sean idénticas, "UNION" elimina duplicadas */


/* 202b. DE CADA EMPLEADO, mostrar su NOMBRE, SALARIO, NOMBRE DEL DEPARTAMENTO 
AL QUE ESTá ASIGNADO Y nombre DE SU DIRECTOR DE DEPARTAMENTO */
...
/* 203. Número de sedes en que está presente cada departamento (identificado por su nombre)*/
...
/* Mostrando también sedes sin departamento */
(	SELECT D.nombre, 
		count(DISTINCT sede) AS sedes
	FROM empleados E RIGHT JOIN departamentos D ON E.departamento=D.num 
	GROUP BY departamento
)
UNION -- Emulación de Full JOIN en caso de querer contemplar empleados sin departamento
(     -- y también departamentos sin empleados
...
/* 204. Número de sedes en que están presentes los departamentos 1 y 6, 
...
/* 206. Número de sedes en que está presentes el departamento "Dep1: Administración" */
...
WHERE D.nombre="Dep1: Administración";
...
/* 211. Listar todos los nombres de familiares, junto con el nombre del empleado 
 del que son familiares (considerando que pudiese haber familiares no asignados a ningún
 empleado) */

SELECT concat(f.nombre, " ", f.apellidos) AS familiar,
	 concat(e.nombre, " ", e.apellidos) AS empleado
FROM ( empleados e JOIN familiares_emp fe ON e.id=fe.empleado )
	RIGHT JOIN familiares f ON f.id=fe.familiar;
...
/* 212- POR CADA PROYECTO QUE TENGA AL MENOS UN EMPLEADO TRABAJANDO EN ÉL
...
/* 214. Como la anterior pero solo para proyectos que aún no han terminado.*/
...
FROM (SELECT SUP.id, SUP.nombre, SUP.apellidos, SUP.departamento
		FROM empleados E INNER JOIN empleados SUP ON E.supervisor=SUP.id
			-- Supervisores son solo los presentes en la FK, así que tiene que ser INNER
		GROUP BY SUP.id) SUP
...
/* --------------------------------------------------------------
 *  216. Supervisores con su salario y con el salario máximo de sus supervisados */
					
	/* Empleados que tienen supervisor */
	SELECT id, nombre, apellidos, supervisor 
	FROM empleados
	WHERE supervisor IS NOT NULL AND supervisor<>id
	-- Si se supervisa a sí mismo no lo consideramos ni supervisor ni supervisado
	...
	/* Salario máximo de los supervisados de cada supervisor */
 ...
 * tengan o no proyectos asignados, o sea, también con aquellos que tengan 0 proyectos */
...
GROUP BY E.id
/* COALESCE() devuelve el primer valor que no sea null de una lista de parámetros
...
/* 224. Mostrar el último empleado en incorporarse a cada proyecto */
...
/* 225. Todos los departamentos con su número de empleados, mostrando también el número
...
UNION -- Emulación de un FULL JOIN
(
